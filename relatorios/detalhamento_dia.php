...
<?php
//echo "<meta HTTP-EQUIV='refresh' CONTENT='60;URL=telao_1.php'>";
?>
</head>
<body>
<?php
...
<table border="1" style="float: left">
  <thead>
  <tr>
    <td colspan="6"><font size="6px"><center> Quantidade de Agendados <?php echo $date; ?> </center></font> </td>
  </tr>
</thead>
<thead>
  <tr>
    ...
    <th><center><b> Agency Concept </b></center></th>
   ...
   <?php 
      ...
      while($row_funcionario = mysqli_fetch_assoc($consulta_funcionario)){
        ...
        $consult_matriz = "SELECT * FROM `agendamentos` where DATE(data_cadastro_agendamento) = '$date' AND id_func = '$id_usuario' AND id_unidade = '1' AND reagendado = '0'";
        $consulta_matriz = mysqli_query($conn, $consult_matriz);
        $num_result_matriz = mysqli_num_rows($consulta_matriz); 

        $consult_lapa = "SELECT * FROM `agendamentos` where DATE(data_cadastro_agendamento) = '$date' AND id_func = '$id_usuario' AND id_unidade = '2' AND reagendado = '0'";
        $consulta_lapa = mysqli_query($conn, $consult_lapa);
        $num_result_lapa = mysqli_num_rows($consulta_lapa); 

        $consult_impact = "SELECT * FROM `agendamentos` where DATE(data_cadastro_agendamento) = '$date' AND id_func = '$id_usuario' AND id_unidade = '3' AND reagendado = '0' ";
        $consulta_impact = mysqli_query($conn, $consult_impact);
        $num_result_impact = mysqli_num_rows($consulta_impact); 

        $consult_num_reagendado = "SELECT * FROM agendamentos WHERE DATE(data_cadastro_agendamento) = '$date' AND id_func = '$id_usuario' AND reagendado = '1'"; 
        $consulta_num_reagendado = mysqli_query($conn, $consult_num_reagendado);
        $num_reagendado = mysqli_num_rows($consulta_num_reagendado);

        $consult_concept = "SELECT * FROM `agendamentos` where DATE(data_cadastro_agendamento) = '$date' AND id_func = '$id_usuario' AND id_unidade = '4' AND reagendado = '0' ";
        $consulta_concept = mysqli_query($conn, $consult_concept);
        $num_result_concept = mysqli_num_rows($consulta_concept); 

    ?>
    <?php
      if($num_result_matriz <> 0 || $num_result_lapa <> 0 || $num_result_impact <> 0 || $num_result_concept <> 0){
  ...
  <tr>
    <td style="
    <?php
      if($row_funcionario['menu_scouter_ligacao'] == 1 && $num_result_matriz >= 30 || $num_result_lapa >= 30 || $num_result_impact >= 30){
        echo 'background-color: blue;color:white;';
      }else if($row_funcionario['menu_scouter_insta'] == 1 && $num_result_matriz >= 15 || $num_result_lapa >= 15 || $num_result_impact >= 15){
        echo 'background-color: blue; color:white;';
      };
    ...
    ?>"><center> <?php echo $num_result_concept; ?> </center> </td>
    <td style="
    <?php
      if($row_funcionario['menu_scouter_ligacao'] == 1 && $num_result_matriz >= 30 | $num_result_lapa >= 30 || $num_result_impact >= 30){
        echo 'background-color: blue;color:white;';
    ...
    <?php
      ...
      $total_dia = $total_matriz + $total_impact + $total_lapa + $total_concept;
    ?>
    <td style="
    <?php
      if($row_funcionario['menu_scouter_ligacao'] == 1 && $num_result_matriz >= 30 | $num_result_lapa >= 30 || $num_result_impact >= 30){
    ...
    ?>"><center> <?php echo $num_result_matriz + $num_result_lapa + $num_result_impact + $num_result_concept; ?> </center></td>
  </tr>
</tbody>
  <?php 
    };
