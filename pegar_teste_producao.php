...
<head>
    ...
    <?php
        echo "<meta HTTP-EQUIV='refresh' CONTENT='20;URL=pegar_teste_producao.php'>";
    ?>
                  ...
                  <table id="bootstrap-data-table" class="table table-striped table-bordered">
                    <thead>
                      <tr>
                        ...
                        if($unidade == 4){
                            $procedimento = "SELECT * FROM acompanhamento_concept ae INNER JOIN cliente cli ON ae.id_cliente = cli.id_cliente WHERE ae.encaminhou_prod = 1";
                            $resultado_procedimento = mysqli_query($conn, $procedimento);
                            $qtd_resultado_procedimento = mysqli_num_rows($resultado_procedimento);
                            
                        }else if($unidade == 1){
                            $procedimento = "SELECT * FROM acompanhamento_exclusive ae INNER JOIN cliente cli ON ae.id_cliente = cli.id_cliente WHERE ae.encaminhou_prod = 1";
                            $resultado_procedimento = mysqli_query($conn, $procedimento);
                            $qtd_resultado_procedimento = mysqli_num_rows($resultado_procedimento);
                        }
                    ...
                    <tbody>
                      <tr>
                        ...
                        echo "<a href='actions/act_procedimento_diario.php?pcdm=8&idpcdm=$idprocedimento'>Atender Cliente</a>" 
                        ?></td>
                      </tr>
                    </tbody>
                    <?php
