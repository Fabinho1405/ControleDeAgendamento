<?php
    ...
    if($linhaAutentica->id_func == $idFuncionario || $conf == 1){
        ...
        if($_GET['acp'] == 1){
             $insereAgendamento=$pdo->prepare("INSERT INTO agendamentos (data_agendada_agendamento, hora_agendada_agendamento, data_cadastro_agendamento, id_cliente, id_status_sistema, id_func, id_comparecimento,id_unidade, reagendado, reagendadoAcompanhamento, id_meio_captado, id_func_conf) VALUES (:dataAgendado,:horaAgendado,NOW(),:idCliente,1,:idFuncionario,3,:idUnidade, 1, 1, :meioCaptado, :funConf)");
        }else{
    	   $insereAgendamento=$pdo->prepare("INSERT INTO agendamentos (data_agendada_agendamento, hora_agendada_agendamento, data_cadastro_agendamento, id_cliente, id_status_sistema, id_func, id_comparecimento,id_unidade, reagendado, reagendadoAcompanhamento, id_meio_captado) VALUES (:dataAgendado,:horaAgendado,NOW(),:idCliente,1,:idFuncionario,3,:idUnidade, 1, 1, 3)");
        };
        ...
        $updateNumeroFB=$pdo->prepare("UPDATE agendamentos SET qtd_fb_acompanhamento=:novoNumero WHERE id_agendamentos=:idAgendamento");
        ...
        $registraLog=$pdo->prepare("INSERT INTO  log_fedback_acompanhamento VALUES (NULL, :funcionario, :nFedback , :idUnidade, NOW() , :idAgendamento, :status)");
