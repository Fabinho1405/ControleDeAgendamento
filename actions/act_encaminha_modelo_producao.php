<?php
	...
	if($unidadefunc == 4){
		...
		if($qtd_select_contrato > 0){
			...
			$insere_solicitacao_estudio = "INSERT estudio_concept (contrato_cc, func_encaminhou_ec, liberado_espera_producao_ec, id_motivo_estudio, obs_func_encaminhou, created) VALUES ('$contrato', '$idfuncionario', '1', '$motivo_encaminhado', '$obsprod',  NOW())";
		...
		if($qtd_select_contrato > 0){
			// Informa que o contrato existe e cadastra na planilha do estúdio a solicitação
			$insere_solicitacao_estudio = "INSERT estudio_exclusive (contrato_cc, func_encaminhou_ec, liberado_espera_producao_ec, id_motivo_estudio, obs_func_encaminhou, created) VALUES ('$contrato', '$idfuncionario', '1', '$motivo_encaminhado', '$obsprod',  NOW())";
			$exec_insert_solicitacao_estudio = mysqli_query($conn, $insere_solicitacao_estudio);
			header("Location: ../index.php");
		}else{
			// Informa que o contrato NÃO foi cadastrado corretamente
