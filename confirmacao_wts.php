<?php
   ...
   if(!empty($_SESSION['id_usuario']) AND $_SESSION['permissao'] != 1){
    ...
    $idunidade = $_GET['select_unidade'];
    $datapresenca = $_GET['datapresenca'];
?>
                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <?php
                                $numero_agendamentos = "SELECT * FROM agendamentos WHERE data_agendada_agendamento = '$datapresenca' AND id_unidade = '$idunidade'";
                            ...
                            ?>
                            <strong class="card-title">Pesquisar</strong> Agendamento | Número de Agendados: <?php echo $qtd_final_agendamentos; ?>
                        </div>
                        <div class="card-body">
                            <center>
                    ...
                    <?php
                            if(!empty($_POST['enviar'])){
                                ...
                                if(empty($tipo)){
                                    ...
                                    if($tipo == 1){                                       
                                        $pesquisa_ficha = "SELECT * FROM agendamentos ag INNER JOIN cliente cli ON ag.id_cliente = cli.id_cliente INNER JOIN funcionario fun ON ag.id_func = fun.id_func WHERE ag.data_agendada_agendamento = '$datapresenca' AND ag.confirmado = '0' AND ag.id_unidade = '$idunidade' AND cli.nome_cliente LIKE '%$parametro%'";
                        ...
                        <td>
                        <form name="contato" action="actions/modificar_confirmacao.php?dataconfirmacao=<?php echo $datapresenca; ?>&idagendamento=<?php echo $row_parametro['id_agendamentos']; ?>&fb=1" method="POST">
                        ...
                        <?php
                                        ...
                                        $pesquisa_ficha = "SELECT * FROM agendamentos ag INNER JOIN cliente cli ON ag.id_cliente = cli.id_cliente INNER JOIN funcionario fun ON ag.id_func = fun.id_func WHERE ag.data_agendada_agendamento = '$datapresenca' AND ag.confirmado = '0' AND ag.id_unidade = '$idunidade' AND cli.telefone_cliente LIKE '%$parametro%'";
                                        $result_parametro = mysqli_query($conn, $pesquisa_ficha);
                                        while($row_parametro = mysqli_fetch_assoc($result_parametro)){
                        ?>
                                        <tr>
