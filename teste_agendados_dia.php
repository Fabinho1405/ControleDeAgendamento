...
<body>
		...
		$select_agendamentos = "SELECT * FROM agendamentos ag 
			...
			INNER JOIN conta_utilizada cont ON ag.id_conta_utilizada = cont.id_conta_utilizada
			WHERE 
			DATE(data_cadastro_agendamento) = '$data_pesquisa' AND 
			ag.id_unidade = '$unidade' AND 
			fun.id_locado = '4' ORDER BY 
			fun.nome_completo_func ASC";
