<?php
	...
	try{
		...
		$mailer->Subject='Fechamento Diário « HOME OFFICE - EXCLUSIVE - '.$dataPrm.' »';

		//Pega Scouter's de 
		
		
		$scouterProducao=$pdo->prepare("SELECT * FROM funcionario WHERE status_sistema=1 AND acesso_direto=1 AND linha_producao=1 AND id_unidade=1");
		...
		";
			...
			foreach($linhaScouters as $rowScouter){
				...
				$selectWts->bindValue(":meioCaptado", 2);
				$selectWts->execute();
