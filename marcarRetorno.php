                      ...
                      <div class="card-body card-block">
                        <form action="actions/act_procedimento_estudio.php?cnt=<?php echo $contrato; ?>&etp=2&acp=<?php echo $procedimento; ?>" method="post" enctype="multipart/form-data" class="form-horizontal">
                          ...
                          <div class="row form-group"> 
                            <div class="col col-md-3"><label class=" form-control-label">Nº de Contrato:</label></div>
                            <div class="col-12 col-md-9">
                              <select name="numeroContrato" id="select" class="form-control">
                                <?php 
                                    if($unidadefunc == 1){

                                    }elseif($unidadefunc == 4){
                                    $pesquisaContrato=$pdo->prepare("SELECT * FROM clientes_concept WHERE status_cc=:status ORDER BY contrato_cc DESC");
                                    }
                                    $pesquisaContrato->bindValue(":status", 12);
                                    $pesquisaContrato->execute();
                                    $linhaContrato=$pesquisaContrato->fetchall(PDO::FETCH_OBJ);
                                    foreach($linhaContrato as $rowContrato){
                                ?>
                                <option value="<?php echo $rowContrato->contrato_cc; ?>"> <?php echo $rowContrato->contrato_cc." - ".$rowContrato->nome_modelo_cc;  ?></option>
                               <?php
                                  }; 
                               ?>
                              </select>
